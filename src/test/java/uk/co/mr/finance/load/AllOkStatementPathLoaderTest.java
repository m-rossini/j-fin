...
public class AllOkStatementPathLoaderTest {
  private static final Logger LOG = LoggerFactory.getLogger(AllOkStatementPathLoaderTest.class);
  ...
  public static void set_up() throws IOException {
    ...
    pairs = tryConnection.map(c -> new StatementPathLoader(c, new InputDataManager(), new LoadControlActions(ctx), new StatementActions(ctx),
                                                           COMMIT_EVERY))
  ...
  private static List<Tuple2<Path, Tuple2<Optional<Throwable>, Optional<StatementSummary>>>> loadFiles(StatementPathLoader loader, Collection<? extends Path> filesToLoad) {
